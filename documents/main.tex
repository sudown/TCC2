...
% RESUMO
\begin{resumo}
O desenvolvimento de software moderno depende de sistemas de controle de versão para integrar modificações paralelas. As ferramentas tradicionais de \textit{merge} não estruturado, como o \textit{diff3}, operam linha a linha e frequentemente reportam falsos conflitos quando alterações ocorrem em linhas adjacentes. Abordagens estruturadas resolvem esse problema utilizando a sintaxe da linguagem, mas possuem alto custo de implementação. Recentemente, a técnica CSDiff (\textit{Custom Separators Diff}) foi proposta como uma alternativa híbrida, utilizando separadores sintáticos para segmentar o código antes da comparação. Este trabalho propõe a ferramenta \textbf{Haskell-SepMerge}, uma adaptação da técnica CSDiff para a linguagem Haskell, cujas características de sintaxe posicional e operadores funcionais desafiam os algoritmos tradicionais. Através de um estudo empírico com repositórios reais, como \textit{ShellCheck}, \textit{Cabal} e \textit{Stack}, demonstrou-se que o Haskell-SepMerge é capaz de reduzir falsos positivos em cenários de inserção adjacente e, em casos específicos, produzir integrações semanticamente superiores às manuais, embora aumente significativamente a granularidade visual dos conflitos.
...
\begin{enumerate}
    ...
    \item Realizar um estudo empírico minerando repositórios reais de Haskell (como \textit{ShellCheck}, \textit{Stack} e \textit{Cabal}) para coletar dados que respondam às QPs;
...
\begin{enumerate}
    \item Clona repositórios Haskell (e.g., \textit{ShellCheck}, \textit{Stack}, \textit{Cabal});
    \item Itera sobre \textit{commits} de merge, reconstruindo as versões Base, Left e Right;
    \item Filtra merges triviais (onde não houve mudança real nos arquivos);
    \item Executa paralelamente o \textit{diff3} e o \textit{Haskell-SepMerge};
    \item Compara os resultados com o \textit{merge} manual (gabarito) utilizando uma comparação ``minificada'' (ignorando espaços em branco) para focar na equivalência semântica e evitar falsos negativos cosméticos.
\end{enumerate}
...
O experimento foi conduzido minerando os repositórios \textit{ShellCheck}, \textit{Cabal} e \textit{Stack}, totalizando a análise de 758 cenários de \textit{merge} conflitantes. Os dados coletados foram consolidados para responder às questões de pesquisa (QPs) definidas na introdução.

\section{QP1: A ferramenta reduz a quantidade total de conflitos?}

Para responder à QP1, contabilizou-se o número total de blocos de conflito reportados por cada ferramenta na amostra analisada. A Tabela \ref{tab:resultados-quantitativos} apresenta o comparativo entre o algoritmo tradicional (\textit{diff3}) e a ferramenta proposta (\textit{Haskell-SepMerge}).

\begin{table}[htb]
\caption{Comparativo de Conflitos e Arquivos Conflitantes (Amostra de 758 cenários)}
...
Cenários Resolvidos (0 conflitos) & 0 & 61 & N/A \\ \hline
\end{tabular}
\legend{Fonte: Elaborado pelo autor com base nos dados do experimento.}
\end{table}

Os dados consolidados revelam um **aumento expressivo na quantidade total de conflitos** (+295,72\%). Este resultado confirma e intensifica a tendência de granularidade observada em estudos similares para Java (+105\%) \cite{clementino2021textual}.

A inclusão do repositório \textit{Stack} foi determinante para este índice. Por se tratar de uma ferramenta de \textit{build}, o código contém extensas definições de registros (\textit{Records}) para configuração. Nestes arquivos, o Haskell-SepMerge fragmentou blocos únicos de conflito em dezenas de micro-conflitos de atribuição.

\section{QP2: A ferramenta reduz a quantidade de cenários com conflitos?}

Apesar da explosão no número de blocos individuais, a segunda linha da Tabela \ref{tab:resultados-quantitativos} demonstra uma **redução de 8,06\% na quantidade de arquivos que exigem atenção manual** (de 757 para 696).

Isso significa que, em **61 cenários**, a granularidade extra permitiu que a ferramenta resolvesse \textbf{todas} as divergências do arquivo automaticamente. Esses são casos onde o desenvolvedor não precisaria abrir o editor para realizar o \textit{merge}, representando um ganho direto de produtividade.

\section{Análise de Falsos Positivos e Negativos (QP3 e QP4)}

\subsection{Sucesso na Redução de Falsos Positivos (QP3)}
Os 61 cenários classificados como "Resolvidos" representam a redução de Falsos Positivos. Nesses casos, o \textit{diff3} reportou conflito, mas o Haskell-SepMerge integrou o código automaticamente. A análise qualitativa de casos como o commit \texttt{3fa5b7d} (\textit{ShellCheck}) confirmou que essa resolução ocorre graças ao isolamento de inserções adjacentes.

\subsection{Análise de Divergência Semântica (QP4)}
A ferramenta gerou alertas de divergência em relação ao \textit{merge} manual. Uma análise qualitativa revelou que, em alguns casos, a ferramenta superou o humano.

Um exemplo emblemático ocorreu no arquivo \texttt{CmdClean.hs} (commit \texttt{6f19128} do \textit{Cabal}). O ramo \textit{Left} renomeou um comando para \texttt{"v2-clean"} e atualizou sua documentação. O ramo \textit{Right} alterou apenas a indentação.
...
\section{Impacto na Legibilidade e Layout}
Observou-se que a inserção de quebras de linha para isolar separadores pode alterar o \textit{layout} do código. Em assinaturas de tipo complexas (com \texttt{=>}), a ferramenta tende a isolar o operador em uma nova linha. Embora o código gerado seja sintaticamente válido para o compilador GHC na maioria dos casos, ele viola as convenções de estilo da linguagem, sugerindo a necessidade de uso conjunto com ferramentas de formatação automática (\textit{code formatters}) no pós-processamento.
...
\begin{itemize}
    \item **QP1 (Total de Conflitos):** O Haskell-SepMerge aumenta a quantidade total (+295\%) devido à granularidade extrema.
    \item **QP2 (Cenários Resolvidos):** A ferramenta reduz o número de arquivos com conflitos pendentes (-8\%).
    \item **QP3 (Falsos Positivos):** A eficácia na redução de falsos positivos foi comprovada em 61 cenários da amostra.
    \item **QP4 (Falsos Negativos):** A ferramenta apresentou divergências em relação ao manual, mas a análise qualitativa indicou que, em casos críticos, o Haskell-SepMerge estava correto e o humano equivocado, embora limitações semânticas (escopo de variáveis) ainda existam.
\end{itemize}

Conclui-se que a ferramenta é viável e segura, recomendando-se seu uso com uma configuração flexível de separadores. Como trabalhos futuros, sugere-se a integração da ferramenta como um \textit{driver} nativo do Git e a incorporação de um formatador automático na etapa de pós-processamento.
